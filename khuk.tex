...
so that in the end data of the order of magnitude of 100~PB \todo{CW:
Are the 100PB yearly volumes or the amount of data to be on disk at any
time? Really?} will be stored to disk.
...
combines several middleware providers \todo{CW: Same text also in KET, is a quote? Appears a strange choice of examples. See comment in KET.)} (ARC, gLite, UNICORE and
...
Analysis frameworks of the large KHuK experiments are usually based on the CERN \todo{ROOT is not entirely a CERN product. So likely politically not correct.} analysis framework ROOT.
...
10.\todo{merge with KET?\\CW: Yes, but adopt to recent status!} 
